...
const tests: Test[] = [
  ...
  test(category + 'create (happy path)', () => {
    ...
    return fotolife.create({ file: path.resolve(__dirname, './bouzuya.png') })
      .then((_) => {
  ...
  test(category + 'create (file does not exist)', () => {
    ...
    return fotolife.create({ file: 'noexists' })
      .then(
        () => assert.fail(),
        (error) => {
  ...
  test(category + 'update (with all options)', () => {
    ...
    return fotolife.update({ id: 123, title: 'TITLE' })
      .then((_) => {
        assert(request.callCount === 1);
  ...
  test(category + 'destroy (with all options)', () => {
    ...
    return fotolife.destroy({ id: 123 })
      .then((_) => {
        assert(request.callCount === 1);
